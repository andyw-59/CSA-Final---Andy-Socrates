public class ElevatorG extends Elevator {
    ...
    public String displayStatus() {
        String status = "[Elevator G] ---------------STATUS--------------- [Elevator G]";
        status +=    "\n |                     Floor range: 0 - 8                     |";
        ...
        status +=    "\n |    Misc: Pass required - Staff/Medical - Local elevator    |  ";
        status +=    "\n +------------------------------------------------------------+";
        ...
        if (inUse) {
            if (client.equals("T") || (client.equals("S") && medicalPass)) {
                if ((currentFloor <= 8 && currentFloor >= 0) && (targetFloor <= 8 && targetFloor >= 0)) {
                    ...
                    double locationPenalty = locationPenalty(side, 5) * (1 - waitTimeFactor);
